...
class LogUniformSampling(DefaultSampling):
    ...
    def get_x_grid(self):
        if self.only_positive:
            return np.logspace(self.range[0], self.range[1], 10000)
        else:
            pos_grid = np.logspace(self.range[0], self.range[1], 5000)
            neg_grid = -np.logspace(self.range[0], self.range[1], 5000)
