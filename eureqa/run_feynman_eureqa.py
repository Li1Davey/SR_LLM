...
@click.command()
@click.argument("results_path", type=str, default="results")  # Path to existing results CSV, to checkpoint runs.
@click.option("--config_path", type=str, default="PATH-TO-CONFIG-FILE", help="Path to Eureqa JSON configuration.")
@click.option("--credential_path", type=str, default="PATH-TO-credential-FILE",
    ...
    if num_workers > 1:
        ...
        for result in pool.imap_unordered(work, args):
            if result is not None:
                pd.DataFrame(result, index=[0]).to_csv(results_path + "/eureqa_result.csv", header=write_header,
                                                       mode='a', index=False)
                write_header = False
    else:
        ...
        for arg in args:
            result = work(arg)
            pd.DataFrame(result, index=[0]).to_csv(results_path + "/eureqa_result.csv", header=write_header, mode='a',
                                                   index=False)
            write_header = False
