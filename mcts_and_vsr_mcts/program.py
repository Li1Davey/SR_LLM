...
class Program(object):
    ...
    def optimize(self, eq, tree_size: int, data_X, y_true, input_var_Xs, eta=0.9999, max_opt_iter=1000, verbose=False):
        ...
        Only C placeholders are allowed.
        Each occurrence of C is fitted independently.
        """
        ...
        # Hard purge of K support.
        if 'K' in eq or 'k_shared' in eq:
            return -np.inf, eq, 0, np.inf

        eq_clean = eq
        num_changing_consts = eq_clean.count('C')
        n_params = num_changing_consts
        ...
        if n_params == 0:
            ...
            return r, eq_clean, t_optimized_constants, t_optimized_obj
        ...
        try:
            ...
            print('\t reward', float(1 / (1 + loss_value) * eta ** tree_size), '\t loss:', loss_value, 'simp:', eq)
...
def execute(expr_str: str, data_X: np.ndarray, input_var_Xs):
    ...
    # Hard purge of K support.
    if 'K' in expr_str or 'k_shared' in expr_str:
        return np.ones(data_X.shape[-1]) * np.inf
