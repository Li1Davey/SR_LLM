...
class KnownEquation(object):
    ...
    def create_dataset(self, sample_size, patience=10, use_control_variable=True):
        ...
        for i in range(patience):
            xs = self.obtain_data_pairs(missed_sample_size * 2)
            y = self.eq_func(xs)
            valid_sample_flags = self.check_if_valid(y)
    ...
    def create_fixedcolumn_dataset(self, sample_size, fixed_column, patience=10):
        warnings.filterwarnings('ignore')
        assert len(self.sampling_objs) > 0, f'There should be at least one variable provided in `{self.sympy_eq}`'
        xs = self.obtain_data_pairs(sample_size)
        for fidx in fixed_column:
            xs[fidx][:] = xs[fidx][0]
        y = self.eq_func(xs)
        # Check if y contains NaN, Infinity, etc
        valid_sample_flags = self.check_if_valid(y)
