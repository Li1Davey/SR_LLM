...
def evaluate(model_path_dict, test_dataset_path_dict, decrements_idx, tau):
    ...
    for test_dataset_key, test_dataset_file_path in test_dataset_path_dict.items():
        ...
        if sympy_eq is None or sympy_eq == sympy.nan:
            ...
            correct_flag_list.append(score > tau)
        else:
    ...
    print(f'Accuracy (R2 > {tau}): {accuracy}% ({num_correct_flags} / {num_samples})')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tau', default=0.999, type=float, help='R2 score threshold')
    ...
    evaluate(model_path_dict, test_dataset_path_dict, args.dec_idx, args.tau)
