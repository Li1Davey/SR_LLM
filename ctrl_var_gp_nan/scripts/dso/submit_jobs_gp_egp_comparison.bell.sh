...
data_path=$basepath/data/unencrypted/equations_dso
py3615=/home/jiang631/workspace/miniconda3/envs/py3615/bin/python3

noise_type=normal
noise_scale=0.0
metric_name=neg_mse
all_equations=`ls $data_path/*.in`
for eq_name in $all_equations;
do
    echo "Submitted $eq_name"
    short_name=$(basename "$eq_name")
    trimed_name=${short_name:0:-3}
   	dump_dir=$basepath/result/DSO/$(date +%F)
    if [ ! -d "$dump_dir" ]
    then
    	echo "create dir: $dump_dir"
    	mkdir -p $dump_dir
	fi
	log_dir=$basepath/log/DSO/$(date +%F)/
	if [ ! -d "$log_dir" ]
	then
